...
function check(): string | undefined {
  ...
  let winner: string;
  const horizontal = (
    row: Array<{ x: number; y: number; letter?: string }>
  ) => {
  ...
  const vertical = (
    square1: { x: number; y: number; letter?: string },
    square2: { x: number; y: number; letter?: string },
    square3: { x: number; y: number; letter?: string }
  ) => {
    const sq = [square1, square2, square3];
    if (sq.every((square) => square.letter !== undefined)) {
      if (
        square1.letter === square2.letter &&
  ...
  if (
    row1[0].letter !== undefined &&
    row2[1].letter !== undefined &&
    row3[2] !== undefined
  ) {
    if (
      row1[0].letter === row2[1].letter &&
  ...
  if (
    row1[2].letter !== undefined &&
    row2[1].letter !== undefined &&
    row3[0] !== undefined
  ) {
    if (
      row1[2].letter === row2[1].letter &&
